...
class _LedgrAppState extends ConsumerState<LedgrApp>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ref.read(recurringServiceProvider).processDueRecurrings().catchError((
        e,
        st,
      ) {
        AppLogger.warning(
          'Error processing recurrings on startup',
          error: e,
          stackTrace: st,
        );
        return 0;
      });
      ref.read(backupServiceProvider).runScheduledBackupIfDue().catchError((
        e,
        st,
      ) {
        AppLogger.warning(
          'Error running scheduled backup on startup',
          error: e,
          stackTrace: st,
        );
        return false;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      ref.read(backupServiceProvider).runScheduledBackupIfDue().catchError((
        ...
        AppLogger.warning(
          'Error running scheduled backup on resume',
          error: e,
          stackTrace: st,
        );
        return false;
